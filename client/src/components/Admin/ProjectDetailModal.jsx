  ...
  useEffect(() => {
    ...
    const fetchSuggestedTeachers = async () => {
      try {
        ...
        // First, try to fetch teacher similarity data from backend
        try {
          const res = await axios.get("/api/admin/teacher-similarity", {
            withCredentials: true,
          });

          console.log("Similarity API Response:", res.data);

          if (res.data && Array.isArray(res.data) && res.data.length > 0) {
            // Find the similarity data for the current project/team
            const teamData = res.data.find((item) => {
              const itemTeamId = item.teamId?.toString() || item.teamId;
              const projectTeamId = (project._id || project.teamId)?.toString();
              
              console.log("Comparing:", itemTeamId, "with", projectTeamId);
            ...
            if (teamData && teamData.teacherScores && teamData.teacherScores.length > 0) {
              // Map the teacher scores to a format suitable for the dropdown
              const teachers = teamData.teacherScores.map((score) => ({
                ...
                // Display name with similarity percentage
          ...
          console.warn("Cosine similarity endpoint failed:", similarityError);
          // Continue to fallback options
        }

        // Fallback 1: Try to fetch all teachers if similarity failed
        console.log("Falling back to fetching all teachers...");
        try {
          ...
          if (teachersRes.data?.success) {
            ...
            if (activeTeachers.length > 0) {
              ...
              toast.info("Showing all available teachers (similarity matching unavailable)");
              return;
            }
          }
        } catch (teachersError) {
          console.warn("Failed to fetch all teachers:", teachersError);
        }

        // Fallback 2: Use project.teachers if available
        console.log("Using project.teachers fallback...");
        const fallbackTeachers = project.teachers || [];
        setSuggestedTeachers(fallbackTeachers);
        
        if (fallbackTeachers.length === 0) {
          toast.warning("No teachers available for assignment");
        }
        ...
        toast.error("Unable to load teacher suggestions");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 p-4">
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-3xl overflow-y-auto max-h-[90vh] shadow-lg">
        ...
        {/* Teacher Rankings Table - Only show if we have similarity data 
